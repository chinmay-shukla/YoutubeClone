...
function Navbar() {
  ...
  return (
    <div className="flex justify-between fixed top-0 w-[100%] z-2 bg-white px-6 py-2 ">
      <div className="flex items-center space-x-4  ">
        <AiOutlineMenu className="text-xl cursor-pointer" onClick={() => { toggleMenu(openMenu) }} />
        <img src={logo} alt="" className="w-28 cursor-pointer" onClick={() => {
          navigate("/");
        }} />
      </div>
      ...
      <div className="flex  space-x-5 items-center ">
        ...
        <Avatar src={profile} size="32" className="cursor-pointer" round={true} onClick={() => { navigate("/profile") }} />
      </div>
    </div>
  );
}

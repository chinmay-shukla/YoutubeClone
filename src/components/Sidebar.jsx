...
function Sidebar() {
  ...
  const handleClick = (e) => {
    ...
    if (e.target.innerHTML == 'Your Channel') {
      navigate('/profile');
    }
  ...
  return (
    <div className="px-5 w-[15%] h-[calc(100vh-6.625rem)] overflow-y-scroll overflow-x-hidden ">
      {/* Home */}
      <div className=" space-y-2 items-center">
        {sidebarItems.map((item) => {
          return (
            <div
              key={item.id}
              className="flex items-center space-x-4 cursor-pointer hover:bg-gray-300 duration-300 rounded-xl p-1"

            >
              <div className="text-xl cursor-pointer">{item.icon}</div>
              <span className="cursor-pointer" onClick={handleClick}>{item.name}</span>
            </div>
          );
        })}
      </div>
